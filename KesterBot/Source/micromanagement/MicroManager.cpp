...
void MicroManager::smartAttackUnit(BWAPI::Unit * attacker, BWAPI::Unit * target) const
{
	...
	if (Options::Debug::DRAW_KESTERBOT_DEBUG) BWAPI::Broodwar->drawLineMap(	attacker->getPosition().x(), attacker->getPosition().y(),
...
void MicroManager::drawOrderText() {
	...
	BOOST_FOREACH (BWAPI::Unit * unit, units) {
		if (Options::Debug::DRAW_KESTERBOT_DEBUG) BWAPI::Broodwar->drawTextMap(unit->getPosition().x(), unit->getPosition().y(), "%s", order.getStatus().c_str());
	}
}
