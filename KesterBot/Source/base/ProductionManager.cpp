...
//	if (Options::Debug::DRAW_KESTERBOT_DEBUG) BWAPI::Broodwar->drawTextScreen(447, 17, "\x07 %d", BuildingManager::Instance().getReservedMinerals());
}
...
void ProductionManager::predictWorkerMovement(const Building & b)
{
	...
	if (Options::Debug::DRAW_KESTERBOT_DEBUG) BWAPI::Broodwar->drawBoxMap(x1, y1, x2, y2, BWAPI::Colors::Blue, false);
...
void ProductionManager::drawProductionInformation(int x, int y)
{
	...
	if (Options::Debug::DRAW_KESTERBOT_DEBUG) BWAPI::Broodwar->drawTextScreen(x, y, "\x04 Build Order Info:");
	...
	for (size_t i(0); i<reps; i++) {
		...
		if (Options::Debug::DRAW_KESTERBOT_DEBUG) BWAPI::Broodwar->drawTextScreen(x, yy, "%s%s", prefix.c_str(), t.getName().c_str());
		if (Options::Debug::DRAW_KESTERBOT_DEBUG) BWAPI::Broodwar->drawTextScreen(x+150, yy, "%s%6d", prefix.c_str(), prod[i]->getRemainingBuildTime());
	}
