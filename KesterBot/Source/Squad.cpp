...
Squad::update()
{
	...
	if (Options::Debug::DRAW_KESTERBOT_DEBUG && order.type == SquadOrder::Attack) 
	{
...
Squad::setNearEnemyUnits()
{
	...
	BOOST_FOREACH(BWAPI::Unit * unit, units)
	{
		...
		if (nearEnemy[unit])
		{
			if (Options::Debug::DRAW_KESTERBOT_DEBUG) BWAPI::Broodwar->drawBoxMap(x-left, y - top, x + right, y + bottom, Options::Debug::COLOR_UNIT_NEAR_ENEMY);
		}
		else
		{
			if (Options::Debug::DRAW_KESTERBOT_DEBUG) BWAPI::Broodwar->drawBoxMap(x-left, y - top, x + right, y + bottom, Options::Debug::COLOR_UNIT_NOTNEAR_ENEMY);
		}
	}
}
